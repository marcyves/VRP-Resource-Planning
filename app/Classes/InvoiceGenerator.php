...
class InvoiceGenerator extends TCPDF
{
    ...
    protected function drawCompanyBox($x, $y, $lineHeight, $h, $company)
    {
        ...
        $addressLines = array_filter([
            $company->address,
            trim($company->zip.' '.$company->city) ?: null,
            $company->country,
            ...
            $company->vat_number ? 'N° de TVA: '.$company->vat_number : null,
        ]);
    ...
    protected function drawSchoolBox($x, $y, $lineHeight, $h, $client)
    {
        ...
        $clientLegalLines = array_filter([
            $client->siret ? 'SIRET: '.$client->siret : ($client->siren ? 'SIREN: '.$client->siren : null),
            $client->vat_number ? 'N° de TVA: '.$client->vat_number : null,
        ]);

        foreach ($clientLegalLines as $line) {
            $currentY += $lineHeight;
            $this->SetXY($x2 + 2, $currentY);
            $this->Cell(0, $lineHeight, $line, 0, 1, 'L', false);
        }
    ...
    public function Footer()
    {
        ...
        // Page number
        $footer = $this->invoice->company->legalFooterLine() ?? '';
        if ($footer !== '') {
     ...
     */
    public function writeLine($x, $y, $h, $line, $justify)
    {
        if ($justify != 'R') {
            $this->SetXY($x, $y);
        }
