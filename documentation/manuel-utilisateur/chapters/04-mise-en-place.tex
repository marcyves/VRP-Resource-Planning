...
\begin{workflowstep}[Programmes]
  Menu \menu{Référentiel} → onglet \menu{Programmes} (\texttt{/program}). Créez une entrée par filière ou offre (ex.~«~Licence Pro~», «~Master Data~»).
\end{workflowstep}
...
\begin{workflowstep}[École]
  Onglet \menu{Écoles} ou bouton d'ajout depuis \texttt{/home}. Renseignez le nom, un \textbf{code} optionnel (unique dans l'entreprise), l'adresse de facturation et les contacts. Un code déjà utilisé par une autre école de la même entreprise est refusé.
\end{workflowstep}
...
\begin{workflowstep}[Cours]
  Depuis la fiche école, ajoutez un cours (\texttt{/course/\{school\_id\}/create}). Associez-le à un programme. Le formulaire compact saisit le volume sur une ligne (\textbf{n sessions de n heures}, total calculé) et le \textbf{tarif horaire en TTC} par défaut (radio HT disponible). La base stocke toujours le tarif \textbf{HT} avec \textbf{4 décimales} pour un aller-retour TTC fidèle (TVA ×\,1{,}2).
\end{workflowstep}

Après \textbf{création}, redirection vers \texttt{/home} (le cours reste sélectionné en session). Après \textbf{modification}, redirection vers la fiche école (\texttt{school.show}), pas vers l'accueil.
...
Sur la fiche école (\texttt{?panel=documents} / \texttt{details})~:

\begin{itemize}[leftmargin=*]
  \item joindre des documents PDF, DOC ou DOCX (max 4~Mo, description obligatoire)~;
  \item maintenir l'adresse et les métadonnées (\texttt{?panel=details})~;
  \item consulter l'historique des factures et la préparation de facturation (\texttt{?panel=courses}, bloc facturation).
\end{itemize}

Les onglets de la fiche~: \textbf{Cours} (défaut, y compris facturation), \textbf{Groupes}, \textbf{Détails}, \textbf{Documents}.
