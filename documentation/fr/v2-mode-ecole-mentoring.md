...
`school.show` est découpé en onglets via `?panel=` (`resources/views/components/school-detail-tabs.blade.php`) :

| Panneau | Query | Contenu |
|---------|-------|---------|
| Cours / activités | `courses` (défaut) | Tableau des cours + bloc facturation |
| Groupes / étudiants | `groups` | Groupes liés actifs / inactifs de l’école |
| Détails | `details` | Adresse / métadonnées (`address` redirige ici) |
| Documents | `documents` | Documents de l’école |

`?focus=billing` force le panneau cours (la facturation y vit). Après édition, la redirection arrive sur `?panel=details`.
...
Sur fiche / édition d’école, changer l’école du breadcrumb doit ouvrir **cette** école (pas revenir à l’URL précédente qui re-lierait la session).

| Entrée | Comportement |
|--------|--------------|
| `PlanningController` / `InvoiceController` `selectSchool` | Réécrit `/school/{id}` et `/school/{id}/edit` vers l’école sélectionnée, en conservant la query |
...
- [Gestion des groupes](gestion-groupes.md) — une formation = un cours vs mentoring multi-activités
